...
void Foam::tractionDisplacementFvPatchVectorField::updateCoeffs()
{
    ...
    const fvPatchField<scalar>& EInf =
        patch().lookupPatchField<volScalarField, scalar>("EInf");

    const fvPatchField<scalar>& nu =
        patch().lookupPatchField<volScalarField, scalar>("nu");

   // scalarField E(rhoE/rho);
    scalarField mu(EInf/(2.0*(1.0 + nu)));
    scalarField lambda(nu*EInf/((1.0 + nu)*(1.0 - 2.0*nu)));
    scalarField threeK(EInf/(1.0 - 2.0*nu));

    Switch planeStress(mechanicalProperties.lookup("planeStress"));

    if (planeStress)
    {
        lambda = nu*EInf/((1.0 + nu)*(1.0 - nu));
        threeK = EInf/(1.0 - nu);
    }
